...
export const HotkeysManager = () => {
  ...
  const handleImportHotkeys = async (importedData: ImportData | Hotkey[]) => {
    try {
      ...
      if (toast) {
        const errorMessage = error instanceof Error ? error.message : "未知错误";
        toast.show({ message: `导入快捷键出错：${errorMessage}`, type: ToastType.error });
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <Dialog open={duplicateConfirmDialog.open} onOpenChange={handleCancelDuplicate}>
        <DialogContent className="bg-neutral-surface">
          <DialogHeader>
            ...
            <DialogDescription>
              快捷键组合"<strong>{duplicateConfirmDialog.newKey}</strong>"已被以下项使用：
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmDuplicate}>允许重复</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
