...
export const useHotkeys = () => {
  ...
  const loadHotkeysFromAPI = useCallback(async () => {
    try {
      ...
      if (toast) {
        toast.show({
          message: "无法从服务器加载自定义快捷键，使用缓存的设置",
  ...
  const handleResetToDefaults = useCallback(() => {
    confirm({
      title: "将快捷键重置为默认值？",
      body: "确定要将所有快捷键和设置重置为默认值吗？此操作无法撤销。",
      okText: "重置为默认值",
      buttonLook: "negative",
      style: { width: 500 },
      onOk: async () => {
        ...
        try {
          ...
          if (result.ok) {
            ...
            if (toast) {
              toast.show({
                message: `保存重置快捷键失败：${result.error || "未知错误"}`,
                type: ToastType.error,
              });
            }
          }
  ...
  const handleImportHotkeys = useCallback(
    async (importedData: ImportData | Hotkey[]) => {
      try {
        ...
        if (!result.ok) {
          throw new Error(result.error || "保存导入的快捷键失败");
        }
